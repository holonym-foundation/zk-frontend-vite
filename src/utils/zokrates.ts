...
import { preprocEndpoint } from '../constants';
import type zokABIs from '../constants/abi/ZokABIs.json';
import { SharedAsyncMemoized } from './SharedAsyncMemoized';

const Preproc = {
  artifacts: async (circuitName: CircuitName) =>
    (await (
      await fetch(`${preprocEndpoint}/${circuitName}Program`)
    ).arrayBuffer()) as Uint8Array,
  provingKey: async (circuitName: CircuitName) =>
    (await (
      await fetch(`${preprocEndpoint}/${circuitName}.proving.key`)
    ).arrayBuffer()) as Uint8Array,
  verifyingKey: async (circuitName: CircuitName) =>
    (await (
...
async function waitForZokProvider(
  retries = 3,
  timeout = 1000
): Promise<ZoKratesProvider> {
  if (zok != null) {
    return zok;
  }
  if (initZok != null) {
...
const cache = new SharedAsyncMemoized(
  async (key: `${CircuitName}.${keyof typeof Preproc}`) => {
    ...
    const result = await Preproc[preprocKey as keyof typeof Preproc](
...
export async function load(
  ...
  return await cache.waitForKey(`${circuitName}.${type}`);
}
...
export const computeWitness = async (circuit: CircuitName, args: unknown[]) => {
  ...
  return zok.computeWitness(input as unknown as CompilationArtifacts, args);
};
