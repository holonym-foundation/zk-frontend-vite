...
describe('useAddNewSecret', () => {
  ...
  it('adds a new secret, newLeaf, and serializedAsNewPreimage to credentials', async () => {
    const { result, waitForNextUpdate } = renderHook(() => useAddNewSecret({ retrievalEndpoint: mockRetrievalEndpoint, newCreds: mockNewCreds }));
    ...
    expect(result.current.newCredsWithNewSecret?.creds.serializedAsNewPreimage).toBeTruthy();
  });
  ...
  it('does not add the same secret to credentials from different retrieval endpoints', async () => {
    const { result, waitForNextUpdate } = renderHook(() => useAddNewSecret({ retrievalEndpoint: mockRetrievalEndpoint, newCreds: mockNewCreds }));
    await waitForNextUpdate();
    const firstNewSecret = result.current.newCredsWithNewSecret?.creds.newSecret;

    const { result: result2, waitForNextUpdate: waitForNextUpdate2 } = renderHook(() => useAddNewSecret({ retrievalEndpoint: 'https://example.com/other-credentials', newCreds: mockNewCreds }));
    await waitForNextUpdate2();
    const secondNewSecret = result2.current.newCredsWithNewSecret?.creds.newSecret;
  ...
  it('does not add the same secret to credentials from the same issuer retrieved at a different time', async () => {
    ...
    const { result: result2, waitForNextUpdate: waitForNextUpdate2 } = renderHook(() => useAddNewSecret({ retrievalEndpoint: mockRetrievalEndpoint, newCreds: mockNewCreds }));
    await waitForNextUpdate2();
    const secondNewSecret = result2.current.newCredsWithNewSecret?.creds.newSecret;
