...
export function useAddNewSecret({ retrievalEndpoint, newCreds }: { retrievalEndpoint: string; newCreds: {
    newLeaf?: string;
    creds: {
      newSecret?: string;
  ...
  const { data: newCredsWithNewSecret, isLoading, error, mutate } = useMutation(
    async () => {
      const newSecret = sessionStorage.getItem(`holoNewSecret-${retrievalEndpoint}`) || generateSecret();
